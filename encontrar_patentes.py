...
_, img_1_bin = cv2.threshold(img_gray.copy(), 160, 255, type=cv2.THRESH_BINARY)
# imshow(img_1_bin)
...
for paty in pats:
    ...
    img_erode = cv2.morphologyEx(filtered_img, cv2.MORPH_DILATE, kernel=e, iterations=1)
    kernel_horizontal = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    img_erode = cv2.morphologyEx(img_erode, cv2.MORPH_ERODE, kernel=kernel_horizontal, iterations=1)
